...
func publisher() {
	...
	if err != nil {
		log.Fatalf("GetUserMedia error: %v", err)
	}

	log.Printf("Stream: %v", mediaStream)
	log.Printf("Got stream with %d video tracks", len(mediaStream.GetVideoTracks()))
	...
	if _, err = room.LocalParticipant.PublishTrack(track, &lksdk.TrackPublicationOptions{
		...
		log.Printf("PublishTrack error: %v", err)
		return
	}
}
